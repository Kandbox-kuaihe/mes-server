...
IS_LIGHT_BUILD = True # os.environ.get("DISPATCH_LIGHT_BUILD") != "0"
...
def get_requirements(env):
    # with open("requirements-{}.txt".format(env), encoding="utf-8") as fp:
    with open(f"requirements-{env}.txt", encoding="utf-8") as fp:
        return [x.strip() for x in fp.read().split("\n") if not x.startswith("#")]


install_requires = get_requirements("base")
dev_requires = get_requirements("dev")
# rl_requires = get_requirements("rl")
...
setup(
    ...
    entry_points={
        ...
        "dispatch.plugins": [
            "dispatch-auth-provider-basic = dispatch.plugins.dispatch_core.plugin:BasicAuthProviderPlugin",
        ],
    },
)
