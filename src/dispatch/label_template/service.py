...
from .models import LabelTemplate, LabelTemplateCreate, LabelTemplateRead, LabelTemplateUpdate, LabelTemplateCreate
...
List[Optional[LabelTemplate]]:
    """Returns an semi given an semi id."""
    return db_session.query(LabelTemplate.id).outerjoin(
        Rolling, LabelTemplate.rolling_id == Rolling.id
    ).filter(
        or_(LabelTemplate.semi_code.like(f'%{semi_code}%'), Rolling.rolling_code.like(f'%{rolling_code}%'))
    ).all() or db_session.query(LabelTemplate.id).outerjoin(
        Area, LabelTemplate.area_id == Area.id
    ).filter(
        or_(LabelTemplate.semi_code.like((f'%{semi_code}%')),
            Area.code.like(f'%{area_code}%')
            )
    ).all() or db_session.query(LabelTemplate.id).outerjoin(
        OrderGroup, LabelTemplate.order_group_id == OrderGroup.id
    ).filter(
        or_(LabelTemplate.semi_code.like((f'%{semi_code}%')),
...
def get(*, db_session, label_template_id: int) -> Optional[LabelTemplate]:
    """Returns an semi given an semi id."""
    return db_session.query(LabelTemplate).filter(LabelTemplate.id == label_template_id).one_or_none()


def get_by_cast_id(*, db_session, cast_id: int) -> List[Optional[LabelTemplate]]:
    """Returns an semi given an semi id."""
    return db_session.query(LabelTemplate).filter(LabelTemplate.cast_id == cast_id).all()


def get_by_spec_ids(*, db_session, spec_ids: List[int]) -> List[Optional[LabelTemplate]]:
    """Returns an semi given an semi id."""
    return db_session.query(LabelTemplate).filter(LabelTemplate.spec_id.in_(spec_ids)).all()


def get_by_code(*, db_session, code: str) -> Optional[LabelTemplate]:
    """Returns an semi given an semi code address."""
    return db_session.query(LabelTemplate).filter(LabelTemplate.code == code).first()


def get_default_semi(*, db_session) -> Optional[LabelTemplate]:
    """Returns an semi given an semi code address."""
    return db_session.query(LabelTemplate).first()


def get_all(*, db_session) -> List[Optional[LabelTemplate]]:
    """Returns all semis."""
    return db_session.query(LabelTemplate)


def create(*, db_session, label_template_in: LabelTemplateCreate) -> LabelTemplate:
    """Creates an semi."""
    contact = LabelTemplate(**label_template_in.dict(
        exclude={"flex_form_data", "mill", "search_vector"}), flex_form_data=label_template_in.flex_form_data)
...
def update(
    ...
    update_data = semi_in.dict(
        exclude={"product_type", "flex_form_data", "mill", "search_vector"})
...
def delete(*, db_session, semi_id: int):
    semi = db_session.query(LabelTemplate).filter(LabelTemplate.id == semi_id).delete()
    db_session.commit()

    return semi


def bulk_get_semi(*, db_session, ids) -> List[Optional[LabelTemplate]]:
    return db_session.query(LabelTemplate).filter(LabelTemplate.id.in_(ids)).all()


def get_area_location(*, db_session, semi_ids) -> Optional[Location]:
    res = db_session.query(LabelTemplate).filter(LabelTemplate.id.in_(semi_ids)).all()
    return res
...
def block_unblock_semi(db_session, semis_in):
    ...
    max_charge_seq = db_session.query(func.max(LabelTemplate.semi_charge_seq)).scalar() or 0
...
def update_semi_by_move(db_session, semi_body):
    try:
        db_session.bulk_update_mappings(LabelTemplate, semi_body)
        db_session.commit()
        return True
...
def find_max_semi_code_by_cast(db_session, semi_code):
    ...
    if not semi_one:
        raise HTTPException(status_code=400, detail="LabelTemplate code not found！")
    cast_id = semi_one.cast_id
    if not cast_id:
        raise HTTPException(status_code=400, detail="Cast id code not found in LabelTemplate！")
    result = db_session.query(LabelTemplate.semi_code).filter(LabelTemplate.cast_id == cast_id).all()
