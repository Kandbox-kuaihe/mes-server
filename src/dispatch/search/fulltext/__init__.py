...
class SQLConstruct(object):
    ...
    def search_vector(self):
        ...
        vectors = (
            ...
            if column_name in self.table.c  # 避免 KeyError
        )
...
def sync_trigger(conn, table, tsvector_column, indexed_columns, metadata=None, options=None):
    ...
    * Updates all rows for given search vector by running a column=column
    ...
    for class_ in classes:
        sql = class_(**params)
        sql_str = str(sql)
        for key in sql.params:
            sql_str = sql_str.replace(f"%({key})s", f":{key}")
        conn.execute(text(sql_str), sql.params)
